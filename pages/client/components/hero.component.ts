    ...
    <section class="relative h-[95vh] overflow-hidden flex items-center justify-center motion-slide-in">
      ...
      <div class="relative z-10 text-center text-white px-6">
        ...
        <div class="flex flex-col md:flex-row justify-center gap-8">
          <button (click)="onDiscoverMap.emit()" class="px-14 py-6 bg-white text-lumina-olive rounded-full font-black text-[11px] uppercase tracking-widest hover:scale-105 transition-all shadow-2xl">Discover Map</button>
          <button (click)="onViewBoutiques.emit()" class="px-14 py-6 border-2 border-white/50 backdrop-blur-md text-white rounded-full font-black text-[11px] uppercase tracking-widest hover:bg-white hover:text-lumina-olive transition-all">250 Houses of Excellence</button>
        </div>
      </div>
    </section>
  `
